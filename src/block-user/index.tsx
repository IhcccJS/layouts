...
const BlockUser: React.FC<any> = (props) => {
  const { avatar, username, role, menus, menuRender, className, style, onMenuClick, renderIcon } = props;
  const { styles, cx } = useStyles();

  const menuNode = Array.isArray(menus) && menus.length > 0 && (
  ...
  return (
    <div className={cx(styles, 'block-user', className)} style={style}>
      <div className={cx(styles, 'block-user-box')}>
        {avatar && <div className={cx(styles, 'block-user-avatar')}>{avatar}</div>}
        <div className={cx(styles, 'block-user-info')}>
          {username && <div className={cx(styles, 'block-user-name')}>{username}</div>}
          {role && (
      ...
      {Array.isArray(menus) && (
        <div className={cx(styles, 'block-user-menu')}>
          <div className={cx(styles, 'block-user-menu-panel')}>{!menuRender ? menuNode : menuRender(menuNode)}</div>
        </div>
      )}
    </div>
  );
